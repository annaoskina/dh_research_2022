...
def count_katakana(filename, parsed_txt):
      k = 1
      katakana_data = ''
                        ...
                        if '外' in token[12]:
                              if 12450 <= ord(token[0][0]) <= 12538: #включаю катакану
                                    if i == 0 or i == 1:
                                          katakana_counter += 1
                                          #eraser = token[7] + '-'
                                          katakana_data = token[0] + '\t' + token[7] + '\t' + token[8].strip(token[7] + '-') + '\t' + token[13] + '\t' + 'katakana' + '\t' + filename + '\t' + token[2] + '\t' + token[3] + '\n'
                                    ...
                                    if i>1:
                                          if parsed_txt[i-1][0] == '《':
                                                ...
                                                if token[0] not in stop_words:
                                                      katakana_counter += 1
                                                      katakana_data = token[0] + '\t' + token[7] + '\t' + token[8].strip(token[7] + '-') + '\t' + token[13] + '\t' + 'katakana' + '\t' + filename + '\t' + token[3] + '\n'
                                                      katakana_list.append(katakana_data)
                                                      #print(i, token[8])
                        elif '固' in token[12]:
                              if 12450 <= ord(token[0][0]) <= 12538: #включаю катакану
                                                      ...
                                                      if len(token[0]) > 3:
                                                            if token[0][1] == token[0][2]:
                                                                  if token[0][2] == token[0][3]:
                                                                        ...
                                                                        katakana_counter += 1
                                                                        katakana_data = token[0] + '\t' + 'None' + '\t' + 'None' + '\t' + 'None' + '\t' + 'katakana' + '\t' + filename + '\t' + token[2] + '\t' + token[3] +  '\n'
                                                                        katakana_list.append(katakana_data)
                                                                        #print(i, token[0], 'hey')
                                          if token[0] not in stop_words:
                                                katakana_counter += 1
...
def count_romaji(filename, parsed_with_ipadic_txt):
      x = 1
      romaji_counter = 0
      romaji_list = []
      romaji_data = ''
      ...
      for token in parsed_with_ipadic_txt:
            ...
            if token[0]:
                  if 65 <= ord(token[0][0]) <= 91: #включаю латиницу half-width (H) заглавные
                        romaji_counter += 1
                        romaji_data = token[0] + '\t' + 'None' + '\t' + token[0] + '\t' + 'None' + '\t' + 'romaji' + '\t' + filename + '\t' + token[1] + '\t' + 'None' + '\n'
...
def count_kanji(filename, parsed_txt):
      x = 1
                        ...
                        if '外' in token[12]:
                              if not 65 <= ord(token[0][0]) <= 122 \
                                 ...
                                 and token[0] not in korean_words:
                                    kanji_counter += 1
                                    kanji_data = token[0] + '\t' + token[7] + '\t' + token[8].strip(token[7] + '-') + '\t' + token[13] + '\t' + 'kanji' + '\t' + filename + '\t' + token[2] + '\t' + token[3] + '\n'
                                          ...
                                          if len(maybe_kanji_token) > 12:
                                                if '外' not in maybe_kanji_token[12]:
                                                      if maybe_kanji_token[0] == '人' or maybe_kanji_token[0] == '語':
                                                            ...
                                                            kanji_counter += 1
                                                            kanji_data = kanji_token[0] + '\t' + kanji_token[7] + '\t' + kanji_token[8].strip(kanji_token[7] + '-') + '\t' + kanji_token[13] + '\t' + 'kanji' + '\t' + filename + '\t' + kanji_token[2] + '\t' + kanji_token[3] + '\n'
                                                            kanji_list.append(kanji_data)
                                                      else: 
                                                            if len(new_token)>7:
                                                                  ...
                                                                  kanji_counter += 1
                                                                  kanji_data = maybe_kanji_token[0] + '\t' + maybe_kanji_token[7] + '\t' + maybe_kanji_token[8].strip(maybe_kanji_token[7] + '-') + '\t' + maybe_kanji_token[13] + '\t' + 'kanji' + '\t' + filename + '\t' + maybe_kanji_token[2] + '\t' + maybe_kanji_token[3] + '\n'
                                                                  kanji_list.append(kanji_data)
                                                            else:
                                                                  furigana = new_token[0]
                                                                  kanji_furigana = maybe_kanji_token[0] + '-' + furigana
...
def main():
      ...
      results = []
      name_csv = 'Taisho_period/taisho_data_filenames.csv'
      for filename in files_list:
            text = read_file(path, filename)
            parsed_ipadic = parse_with_ipadic(text)
            parsed_text = parse_with_kindai(text)
            filename = filename.strip('.txt')
            katakana_list = count_katakana(filename, parsed_text) #возвращает список слов
            #print(filename, 'katakana: ', katakana_list, '\n')
            romaji_list = count_romaji(filename, parsed_ipadic)
            #print(filename, 'romaji: ', romaji_list, '\n')
            kanji_list = count_kanji(filename, parsed_text)
            #print(filename, 'kanji', kanji_list, '\n')
      ...
      with open('/home/annaoskina/DH_research_21_22/git_research_2021_22/tsubouchi_result_gairaigo.tsv', 'w') as out_file:
